...
public:
    ...
    BenchMark(const char* _PATH, const COUNT_TYPE _T):
     ...
     //   FILE* file = fopen(PATH, "rb");
		char errbuf[100];
		pcap_t* file = pcap_open_offline(const_cast<char*>((s + ".pcap").c_str()), errbuf);
		if (file == NULL) {
        ...
        while(pcap_next_ex(file, &pkthdr, &pktdata) == 1){
            if(number % LENGTH == 0)
                T += 1;
            number += 1;
			ip_hdr* iph_ptr = (ip_hdr*)pktdata;
			uint32_t src = ntohl(iph_ptr->srcaddr);
			uint32_t dst = ntohl(iph_ptr->dstaddr);
